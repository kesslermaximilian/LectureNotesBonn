...
\begin{definition}
    Eine \vocab{Metrik} auf einer Menge $X$ ist eine Funktion  $d: X\times X \to  \R_{\geq 0}$ mit folgenden Eigenschaften:
    \begin{enumerate}[(i)]
...
\end{definition}
\begin{definition}
    Seien $(X,d)$ und  $(X',d')$ zwei metrische Räume. Dann ist eine Funktion $f:X \to  Y$ \vocab{stetig in $x\in X$}, falls
    \[
        \forall ε > 0 \; \exists \delta > 0 \; \forall x' \colon d(x,x') < \delta \implies d'(f(x), f(x')) < ε
    .\] 
    Eine Funktion $f$ heißt \vocab{stetig}, wenn sie in jedem Punkt  $x\in X$ stetig ist.
    \begin{minipage}{\textwidth}
        \centering
    \incfig{definition-von-stetigkeit-in-metrischen-raeumen}
    \captionof{figure}{Definition von Stetigkeit in metrischen Raeumen}
    \end{minipage}
\end{definition}
...
\item Sei $X$ eine Menge. Dann ist
    \[
        ...
        \end{cases}
    .\] 
    eine Metrik auf $X$, genannt die \vocab{diskrete Metrik}.
    \end{itemize}
\end{example}
\begin{notation}
    ...
    .\] 
    und nennen dies den \vocab{offenen $ε$-Ball um $x$}
\end{notation}
\begin{observe}
...
\end{observe}
\begin{definition}
    Sei $X$ ein metrischer Raum,  $U\subset X$ und $x\in X$. Dann heißt $U$ \vocab{Umgebung von $x$}, falls ein $ε>0$ existiert, sodass  $U(x,ε) \subset U$. 
\end{definition}
\begin{theorem}
...
\begin{definition}
    Sei $X$ ein metrischer Raum. Eine Teilmenge  $X\subset X$ heißt \vocab{offen}, falls sie Umgebung all ihrer Punkte ist, d.h. $\forall x\in U \;\exists ε>0$ mit $U(x,ε)\subset U$.
\end{definition}
\begin{remark}
    $U(x,ε)$ ist offen.
\end{remark}
...
\begin{definition}
    Eine Topologie auf einer Menge $X$ ist eine Menge  $\Topo$ von Teilmengen von  $X$, so dass gilt:
    \begin{enumerate}[1)]
    ...
    \end{enumerate}
    Die Mengen in $\Topo$ heißen \vocab{offene Mengen}. \\
    Ein \vocab{topologischer Raum} ist ein Paar  $(X,\Topo)$ aus einer Menge  $X$ und einer Topologie  $\Topo$ auf  $X$.
\end{definition}


\begin{definition}
    Seien $X,Y$ topologische Räume. Eine Abbildung  $f:X \to  Y$ heißt \vocab{stetig}, falls für jede offene Teilmenge $U\subset Y$ das Urbild $f^{-1}(U) \subset X$ offen ist.
\end{definition}
...
\begin{example}
    ...
    .\] 
    ein topologischer Raum. $\Topo$ ist die von der Metrik  $d$ \vocab{indupierte Topologie}.
\end{example}
\begin{definition}
    Ein topologischer Raum heißt \vocab{metrisierbar}, falls die Topologie von einer Metrik induziert ist.
\end{definition}

\begin{example}
    Sei $X$ eine Menge. Die \vocab{diskrete Topologie} auf $X$ ist die Menge aller Teilmengen, d.h.  $\Topo := \mathcal{P}(X)$. Diese ist von der diskreten Metrik
    \[
        d(x,y) = \begin{cases}
            0 & \text{falls }x=y \\
            1 & \text{sonst}
...
\begin{example}
    \begin{enumerate}[1)]
        ...
        \item Sei $X$ eine Menge. Die  \vocab{indiskrete Topologie} auf $X$ enthält nur  $\emptyset,X$. Man prüft leicht, dass dies eine Topologie ist. 
            \begin{itemize}
                \item 
