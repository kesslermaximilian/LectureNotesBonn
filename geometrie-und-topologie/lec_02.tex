...
\begin{definition}
    Zwei Metriken $d_1,d_2$ auf $X$ heißen \vocab{äquivalent}, falls Konstanten $c_1,c_2$ existieren, sodass
    \[
...
\begin{definition}
    Sei $X$ ein topologischer Raum und $U\subset X$ sowie $x\in X$. Dann heißt \vocab{$U$ Umgebung} von $x$, falls es eine offene Teilmenge  $O\subset X$ gibt, mit $x\in O\subset U$.
\end{definition}
\begin{remark}
...
\begin{definition}
    Sei $X$ ein topologischer Raum. Eine Teilmenge  $A\subset X$ heißt \vocab{abgeschlossen}, falls ihr Komplement $X \setminus A = \left \{x\in X \mid  x\not\in A\right\} $ offen ist.
\end{definition}
\begin{remark}
...
\begin{theoremdef}
    ...
    .\] 
    eine Topologie auf $Y$, die  \vocab{Teilraumtopologie} oder auch \vocab{Unterraumtopologie} genannt wird.  
\end{theoremdef}
...
\begin{definition}
    ...
    Existiert solch ein $f$, so heißen  $X,Y$  \vocab{homöomorph} 
\end{definition}
\begin{example}
...
.\] 
als die \vocab{Menge der Äquivalenzklassen} von $X$ bezüglich  $\sim $. Definiere nun
    \begin{equation*}
    ...
    \end{array} \right.
\end{equation*}
als die \vocab{kanonische Projektion} von $X$ auf seine Äquivalenzklassen. Wir stellen fest, dass $q$ surjektiv ist.
...
\begin{theoremdef}
    ...
    .\] 
    eine Topologie auf $X / \sim $, genannt die \vocab{Quotiententopologie}. 
\end{theoremdef}
\begin{proof}
