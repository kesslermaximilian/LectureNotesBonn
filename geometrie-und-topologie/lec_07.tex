...
.\] 
$β(X)$ ist kompakt und Hausdorffsch. $β(X)$ heißt  \vocab{Stone-Čech-Kompaktifizierung}.
\begin{warning}
...
\[
    \hat{f}(β(X)) = \hat{f}(\overline{ι_X(X)}) \subset \overline{(\hat{f} \circ  ι_X)(X)} = \overline{(ι_k \circ f)(X)} \subset \overline{ι_K(K)} = ι_K(K) \cong K
.\] 
\end{proof}
