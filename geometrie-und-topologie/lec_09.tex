...
\begin{ddefinition}[Weg]\label{def:weg}
    ...
    \begin{minipage}{\textwidth}
        \centering
    \incfig{weg-in-topologischem-raum}
    \end{minipage}
\end{ddefinition}
...
\begin{example}
   ...
   \item $\R^n \setminus \left \{0\right\} , n\geq 2$ ist wegzusammenhängend. Seien hierzu $x,y\in \R^n \setminus \left \{0\right\}$.
       ...
       \end{description}
    \end{enumerate}
    \missingfigure{Strecken in $\R^n$ skizzieren}
\end{example}
...
\missingfigure{Versuch / Jordankurve skizzieren}
...
\begin{dexample}[Auf Nachfrage in der Vorlesungspause besprochen]
    ...
    \missingfigure{Sternförmige Menge skizzieren}
\end{dexample}

\begin{comment}
\begin{figure}[ht]
    \centering
    \incfig{sternförmige-menge}
    \caption{sternförmige Menge}
...
\begin{theorem}[Lemma von Urysohn]\label{thm:urysohn}
...
\begin{lemma}\label{lm:stetige-abbildung-durch-familie-von-rationalen-offenen-mengen}
    ...
    \missingfigure{Funktion skizzieren}
\end{lemma}
\begin{proof}
    Definiere
        \begin{equation*}
