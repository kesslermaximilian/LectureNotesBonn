...
\[
    A \cup = (A^{c} \cap B^{c})^{c}
.\] 
ebenfalls in $\mathcal{F}$ sein wird. Eine Menge $\mathcal{F}$ mit solchen Eigenschaften heißt \vocab{Algebra}, d.h. wir fordern von $\mathcal{F}$, dass es sich um eine solche Algebra handelt.  \\
...
\begin{definition}
    ...
    \end{enumerate}
    Dann nennen wir $\mathcal{F}$ eine \vocab{$\sigma$-Algebra} und das Paar $\Omega, \mathcal{F})$ einen \vocab{Messraum}. 
\end{definition}
...
\begin{proof}
    ...
    \end{enumerate}
\end{proof}
\begin{corollary}[Einschluss-Ausschluss-Prinzip]
